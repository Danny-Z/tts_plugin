...
class FlutterTts {
  static const MethodChannel _channel = const MethodChannel('tts_plugin');
  ...
  Future<dynamic> setIosAudioCategory(
      IosTextToSpeechAudioCategory category, List<IosTextToSpeechAudioCategoryOptions> options) async {
    ...
    try {
      return _channel.invokeMethod<dynamic>('setIosAudioCategory', <String, dynamic>{
        iosAudioCategoryKey: categoryToString[category],
        iosAudioCategoryOptionsKey: options.map((o) => optionsToString[o]).toList(),
      });
    } on PlatformException catch (e) {
      print('setIosAudioCategory error, category: $category, error: ${e.message}');
    }
  }
  ...
  /// [Future] which invokes the platform specific method for setVoice
  /// ***Android supported only***
  Future<dynamic> setEngine(String enginePkg) => _channel.invokeMethod('setEngine', enginePkg);
  ...
  Future<List<String>> get getLanguages async {
    return ((await _channel.invokeMethod('getLanguages')) as List<dynamic>).map((e) => e as String).toList();
  }

  /// [Future] which invokes the platform specific method for getEngines
  /// Returns a list of installed TTS engines
  /// ***Android supported only***
  Future<List<String>> get getEngines async {
    return ((await _channel.invokeMethod('getEngines')) as List<dynamic>).map((e) => e as String).toList();
  }

  /// [Future] which invokes the platform specific method for getVoices
  /// Returns a `List` of voice names
  Future<List<String>> get getVoices async {
  ...
  Future<SpeechRateValidRange> get getSpeechRateValidRange async {
    ...
    final platform = TextToSpeechPlatform.values.firstWhere((e) => describeEnum(e) == platformStr);
