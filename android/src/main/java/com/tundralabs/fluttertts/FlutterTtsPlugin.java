...
public class FlutterTtsPlugin implements MethodCallHandler, FlutterPlugin {
  ...
  private static boolean isTtsInitFailed = false;
  private ArrayList<Runnable> initMethodCalls = new ArrayList<>();
  ...
  private void initInstance(BinaryMessenger messenger, Context context) {
    ...
    methodChannel = new MethodChannel(messenger, "tts_plugin");
  ...
  public void onMethodCall(@NonNull final MethodCall call, @NonNull final Result result) {
    ...
    if(isTtsInitialized && call.method.equals("enable")){
      result.success(isTtsInitFailed ? 0 : 1);
      return;
    }
    if (!isTtsInitialized) {
      if(call.method.equals("enable")){
        ...
        initMethodCalls.add(suspendedCall);
      } else {
    ...
    if(isTtsInitFailed){
      result.error("tts init failed", "tts not support", null);
      return;
    }
    switch (call.method) {
      case "speak":
  ...
  void setEngine(final String engine,final Result result){
    ...
    initMethodCalls.add(new Runnable() {
      @Override
      public void run() {
        result.success(isTtsInitFailed ? 0 : 1);
      }
    ...
    try {
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
        ...
        for (Locale locale : Locale.getAvailableLocales()) {
          if (locale.getVariant().isEmpty() && isLanguageAvailable(Locale.forLanguageTag(locale.toLanguageTag()))) {
            locales.add(locale.toLanguageTag());
          }
        }
      }
